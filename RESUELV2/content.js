...
function init() {
  ...
  async function analyzeFormWithAI(form){
    ...
    try {
      ...
      const res = await chrome.runtime.sendMessage({ type: 'ANALYZE_FORM', html: `${html}\n\nADDITIONAL_CONTEXT_JSON:${contextJSON}` });
  ...
  // Ask by Photo Modal Function - Professional Chat UI
  async function openAskByPhotoModal() {
    ...
    try {
      ...
      // Create Ask by Photo modal following za-modal structure (chat look)
      const modal = document.createElement('div');
      modal.id = 'zepra-ask-photo-modal';
      modal.innerHTML = `
        <div class="apbp-shell">
          <div class="apbp-modal">
            <header class="apbp-header">
              <div class="apbp-title">
                <span class="apbp-logo">🖼️</span>
                <span>Ask by Photo</span>
              </div>
            ...
            <footer class="apbp-footer">
              <div class="apbp-tools">
                <button id="apbpUpload" class="tool"><span>Upload</span></button>
                <button id="apbpFull" class="tool"><span>Full Page</span></button>
                <button id="apbpPartial" class="tool"><span>Partial</span></button>
              </div>
              <div class="apbp-input">
                <input id="apbpText" type="text" placeholder="Ask a question about the image..." />
                <button id="apbpSend" class="send" disabled>Send</button>
              </div>
            </footer>
          </div>
        </div>`;
      ...
      style.textContent = `
        ...
        .apbp-shell{position:fixed;inset:0;background:rgba(0,0,0,.55);backdrop-filter:blur(4px);z-index:2147483647;display:flex;align-items:center;justify-content:center;animation:fadeIn .25s ease}
        .apbp-modal { background:linear-gradient(145deg,#0f172a 0%,#0b1220 100%);border-radius:18px;width:min(680px,94vw);height:min(560px,90vh);display:flex;flex-direction:column;box-shadow:0 25px 60px rgba(0,0,0,.6);border:1px solid rgba(255,255,255,.08) }
        .apbp-header{display:flex;align-items:center;justify-content:space-between;padding:12px 14px;border-bottom:1px solid rgba(255,255,255,.08)}
        .apbp-title{display:flex;align-items:center;gap:10px;color:#e2e8f0;font-weight:700}
        .apbp-logo{width:26px;height:26px;display:grid;place-items:center;background:rgba(59,130,246,.15);border:1px solid rgba(59,130,246,.35);border-radius:9999px}
        .apbp-close{background:transparent;border:none;color:#e2e8f0;width:36px;height:36px;border-radius:9999px;cursor:pointer}
        .apbp-close:hover{background:rgba(255,255,255,.06)}
        .apbp-body{flex:1;overflow:auto;padding:14px}
        .apbp-messages{display:flex;flex-direction:column;gap:12px}
        .apbp-footer{border-top:1px solid rgba(255,255,255,.08);padding:10px;display:flex;flex-direction:column;gap:10px}
        .apbp-tools{display:flex;gap:8px}
        .apbp-tools .tool{background:rgba(148,163,184,.14);color:#cbd5e1;border:1px solid rgba(148,163,184,.25);border-radius:10px;padding:8px 12px;cursor:pointer}
        .apbp-tools .tool:hover{background:rgba(148,163,184,.20)}
        .apbp-input{display:flex;gap:8px}
        .apbp-input input{flex:1;background:rgba(2,6,23,.6);border:1px solid rgba(59,130,246,.35);border-radius:12px;color:#e2e8f0;padding:10px 12px}
        .apbp-input .send{background:#22c55e;border:none;color:#052e1b;border-radius:12px;padding:10px 14px;cursor:pointer}
        .apbp-input .send:disabled{opacity:.5;cursor:not-allowed}
        .apbp-message{display:flex;gap:10px}
        .apbp-message.ai .apbp-message-avatar{background:rgba(59,130,246,.18);border:1px solid rgba(59,130,246,.35)}
        .apbp-message.user .apbp-message-avatar{background:rgba(34,197,94,.18);border:1px solid rgba(34,197,94,.35)}
        .apbp-message-avatar{width:28px;height:28px;border-radius:9999px;display:grid;place-items:center;color:#e2e8f0}
        .apbp-message-content{background:rgba(15,23,42,.7);border:1px solid rgba(148,163,184,.20);border-radius:12px;padding:10px 12px;color:#e2e8f0;max-width:88%}
        .apbp-message-content img{max-width:180px;border-radius:10px;margin-bottom:6px;display:block}
      `;
      ...
      const t = {
        en: {
          title: 'Ask by Photo',
          welcome: "Hello! I'm ready to help you analyze images. Upload an image, take a screenshot, or capture a specific area, then ask me anything about what you see!",
          upload: 'Upload', full: 'Full Page', partial: 'Partial', placeholder: 'Ask a question about the image...', send: 'Send',
          added: 'Image attached. What would you like to know?',
          capturing: 'Capturing...'
        },
        ar: {
          title: 'اسأل بالصورة',
          welcome: 'مرحبًا! أستطيع مساعدتك في تحليل الصور. ارفع صورة، التقط الشاشة كاملة أو جزءًا منها، ثم اسألني عما تراه!',
          upload: 'رفع', full: 'كامل الصفحة', partial: 'تحديد جزء', placeholder: 'اكتب سؤالك عن الصورة...', send: 'إرسال',
          added: 'تم إرفاق الصورة. ماذا تريد أن تعرف؟',
      ...
      const textInput = modal.querySelector('#apbpText');
      const sendBtn = modal.querySelector('#apbpSend');
      const uploadBtn = modal.querySelector('#apbpUpload');
      const fullBtn = modal.querySelector('#apbpFull');
      const partialBtn = modal.querySelector('#apbpPartial');
      ...
      function addMessage(type, text, imageData = null) {
        ...
        msg.className = `apbp-message ${type}`;
        ...
        if (imageData && type === 'user') {
          ...
          img.src = imageData;
          img.style.cssText = 'max-width: 180px; border-radius: 10px; margin-bottom: 6px; box-shadow: 0 4px 12px rgba(0,0,0,.3)';
      ...
      addMessage('ai', t.welcome);
      // Localize labels
      modal.querySelector('.apbp-title span:last-child').textContent = t.title;
      uploadBtn.querySelector('span').textContent = t.upload;
      fullBtn.querySelector('span').textContent = t.full;
      partialBtn.querySelector('span').textContent = t.partial;
      ...
      uploadBtn.addEventListener('click', () => {
        ...
        fileInput.addEventListener('change', (event) => {
          ...
          reader.onload = (e) => {
            ...
            addMessage('user', t.added, currentImage);
            updateSendButton();
      ...
      // Full Page capture: scroll+stitch without OCR
      fullBtn.addEventListener('click', async () => {
        try {
          sendBtn.classList.add('processing');
          const dataUrl = await captureFullPageStitched();
          addMessage('user', t.added, dataUrl);
          currentImage = dataUrl;
        } catch(e){
          showNotification('Capture failed: '+e.message, 'error');
        } finally { sendBtn.classList.remove('processing'); }
      });
      // Partial capture: hide modal, select area overlay, capture visible and crop
      partialBtn.addEventListener('click', async () => {
        try{
          const shell = modal.querySelector('.apbp-shell');
          shell.style.display = 'none';
          const region = await selectAreaOverlay();
          const cap = await chrome.runtime.sendMessage({ type:'CAPTURE_VISIBLE' });
          if(!cap?.ok) throw new Error(cap?.error||'capture');
          const cropped = await cropDataUrl(cap.dataUrl, region);
          shell.style.display = '';
          addMessage('user', t.added, cropped);
          currentImage = cropped;
        }catch(e){
          // Restore panel if hidden
          modal.querySelector('.apbp-shell').style.display = '';
          showNotification('Partial capture failed: '+e.message, 'error');
        }
      });
      ...
      function updateSendButton() {
        ...
        sendBtn.disabled = !hasText; // سؤال بدون صورة مسموح
      }
      ...
      async function handleSendMessage() {
        ...
        try{
          const res = await chrome.runtime.sendMessage({ type:'ASK_TEXT', question });
          const reply = res?.ok ? (res.text || 'OK') : (res?.error || 'Error');
          stopTyping();
          addMessage('ai', formatAIReply(reply));
        }catch(e){
          stopTyping();
      ...
      // Auto-resize input (treated as textarea-feel)
      ...
      function showTyping(){
        ...
        el.className = 'apbp-message ai';
        el.innerHTML = '<div class="apbp-message-avatar">AI</div><div class="apbp-message-content"><span class="dots"><span>.</span><span>.</span><span>.</span></span></div>';
      ...
      function formatAIReply(text){
        // lightweight formatting: split by two newlines into sections
        const parts = String(text||'').split(/\n\n+/).map(p=>p.trim()).filter(Boolean);
        if(parts.length<=1) return text;
        let html = '';
        const titles = ['Summary','Details','Notes'];
        parts.forEach((p,i)=>{ html += `<div><strong>${titles[i]||''}</strong><div>${p.replace(/\n/g,'<br/>')}</div></div>`; });
        return html;
      }
      ...
      async function captureFullPageStitched(){
        ...
        for(let i=0;i<steps;i++){
          ...
          if(!r?.ok) throw new Error(r?.error||'capture');
          images.push(r.dataUrl);
        }
        window.scrollTo(0, originalScrollY);
        // Stitch
        const canvas = document.createElement('canvas');
        const img0 = await loadImg(images[0]);
        const ratio = img0.width / window.innerWidth;
      ...
      async function selectAreaOverlay(){
        return new Promise((resolve, reject)=>{
          const ov = document.createElement('div');
          ov.style.cssText='position:fixed;inset:0;cursor:crosshair;background:rgba(0,0,0,.08);z-index:2147483646';
          const box = document.createElement('div');
          box.style.cssText='position:absolute;border:2px solid #4ade80;background:rgba(74,222,128,.12)';
          let sX=0,sY=0,drag=false;
          ov.addEventListener('mousedown',(e)=>{ drag=true; sX=e.clientX; sY=e.clientY; box.style.left=sX+'px'; box.style.top=sY+'px'; box.style.width='0px'; box.style.height='0px'; if(!box.parentNode) ov.appendChild(box); });
          ov.addEventListener('mousemove',(e)=>{ if(!drag) return; const x=Math.min(e.clientX,sX), y=Math.min(e.clientY,sY); const w=Math.abs(e.clientX-sX), h=Math.abs(e.clientY-sY); Object.assign(box.style,{left:x+'px',top:y+'px',width:w+'px',height:h+'px'}); });
          const finish=(e)=>{ if(!drag){ cleanup(); reject(new Error('cancelled')); return; } drag=false; const rect=box.getBoundingClientRect(); cleanup(); resolve({ x: rect.left, y: rect.top, width: rect.width, height: rect.height, scrollX: window.scrollX, scrollY: window.scrollY }); };
          const cancel=(e)=>{ cleanup(); reject(new Error('cancelled')); };
          ov.addEventListener('mouseup', finish, { once:true });
          ov.addEventListener('mouseleave', cancel, { once:true });
      ...
      messagesContainer.addEventListener('click',(e)=>{
        const img = e.target.closest('.apbp-message-content img');
        if(!img) return;
        const lb = document.createElement('div');
        lb.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.85);display:flex;align-items:center;justify-content:center;z-index:2147483647;animation:fadeIn .2s ease';
        const im = new Image(); im.src = img.src; im.style.cssText='max-width:92vw;max-height:92vh;border-radius:12px;box-shadow:0 20px 80px rgba(0,0,0,.6)';
        lb.appendChild(im);
  ...
  function showIPQualificationModal(data){
    ...
    const warningSVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" class="warning-icon"><path d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>`;
  ...
  async function showFullIPModal(info){
    const data = info || {};
    ...
    const icons = {
      ...
      map:`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 6l7-3 7 3 7-3v15l-7 3-7-3-7 3z"/><path d="M8 3v15"/><path d="m12 17 .01 0"/></svg>`,
      globe,
  ...
  function extractJSON(text){
    try {
      ...
      console.log('extractJSON - trying to parse:', jsonStr);
      
      const result = JSON.parse(jsonStr);
      console.log('extractJSON - parsed successfully:', result);
      return result;
    } catch (e) {
      console.log('extractJSON - parse failed:', e.message);
      return null;
    }
  }
    ...
    if (withReason) {
      if (obj && (obj.answer || obj.reason)) {
        ...
        return {
          ...
          reason: 'AI response format was not as expected.'
        };
      }
    }
...
function showCleanIPQualificationModal(data) {
  ...
  async function openAskByPhotoModal() {
    ...
    try {
      ...
      console.log('Creating modal element...');
      // Create Ask by Photo modal with chat-like interface
      const modal = document.createElement('div');
      modal.id = 'zepra-ask-photo-modal';
      modal.innerHTML = `
        <div class="apbp-modal">
      ...
      addMessage('ai', 'Hello! I\'m ready to help you analyze images. You can upload an image, take a full page screenshot, or capture a specific area of the page. Then ask me anything about what you see!');
      ...
      fullScreenBtn.addEventListener('click', () => {
        showNotification('Full page screenshot feature will be implemented in the next update!', 'info');
      });

      partialScreenBtn.addEventListener('click', () => {
        showNotification('Partial screenshot feature will be implemented in the next update!', 'info');
      });
      ...
      function handleSendMessage() {
        ...
        setTimeout(() => {
          addMessage('ai', 'Thank you for your question! The AI image analysis feature will be fully connected in the next update. For now, this demonstrates the chat interface working correctly.');
        }, 1000);
      }
