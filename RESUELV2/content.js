  ...
  async function openAskModal() {
    ...
    const buildChatPrompt = (history) => {
      const formatted = history
        ...
        .join('\n');
      return `You are Zepra, a professional, friendly assistant. Answer clearly and concisely.\n- Match the user's language.\n- Keep answers inside this chat window.\n- Avoid markdown unless the user requests it explicitly.\n\nConversation:\n${formatted}\n\nAssistant:`;
    };

    addMessage('ai', 'مرحباً! اكتب سؤالك وسأجيبك هنا داخل نفس النافذة.');
    ...
    askSubmit.addEventListener('click', async () => {
      ...
      try {
        ...
        typingMessage.remove();
        addMessage('ai', 'حدث خطأ أثناء توليد الرد. حاول مرة أخرى.');
